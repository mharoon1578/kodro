...
def init(
    ...
    # Generate authoritative integration bootloader with command parser
    from kodro.constants import BOOTLOADER_TEMPLATE
    bootloader = BOOTLOADER_TEMPLATE
    # Replace integration-specific command patterns
    if integration.value == "claude":
        bootloader = bootloader.replace("$kodro", "$kodro").replace("/kodro", "/kodro")
    elif integration.value == "opencode":
        bootloader = bootloader.replace("$kodro", "/kodro").replace("/kodro", "/kodro")
    else:
        bootloader = bootloader.replace("$kodro", f"/{integration.value}").replace("/kodro", f"/{integration.value}")
    integration_file = integration_dir / INTEGRATION_FILES[integration.value]
    write_file(integration_file, bootloader)
